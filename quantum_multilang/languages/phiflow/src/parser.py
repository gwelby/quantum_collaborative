...
This module provides parsing functionality for the φFlow DSL,
...
class PhiFlowSyntaxError(Exception):
    """Exception raised for syntax errors in φFlow DSL code."""
    pass

class PhiFlowParser:
    """Parser for φFlow DSL."""
    ...
    def __init__(self):
        ...
        self.token_patterns = [
            ...
            ('COMPRESSION', r'compression\s*=\s*([0-9.]+|φ\^?[0-9.]*)'),
    ...
    def tokenize(self, code: str) -> List[Tuple[str, str, int]]:
        """
        Convert φFlow code into a list of tokens.
    ...
    def parse(self, code: str) -> List[Union[Dict[str, Dict[str, Any]], List[StateTransition]]]:
        """
        Parse φFlow code into a list of state transitions and definitions.
            ...
            if token_type == 'COMMENT':
                ...
                while j < len(tokens):
                    ...
                    if prop_type == 'FREQ':
                        ...
                        compression_match = re.match(r'compression\s*=\s*([0-9.]+|φ\^?[0-9.]*)', prop_value)
                        compression_val = compression_match.group(1)
                        # Parse phi expressions
                        if 'φ' in compression_val:
                            if '^' in compression_val:
                                power = float(compression_val.split('^')[1])
                                states[state_name]['compression'] = f"PHI^{power}"
                            else:
                                states[state_name]['compression'] = "PHI"
                        else:
                            states[state_name]['compression'] = float(compression_val)
                ...
                while j < len(tokens):
                    ...
                    if prop_type == 'WHEN':
                        ...
                        # Handle phi values
                        if 'φ' in op_value or 'Φ' in op_value:
                            op_value = op_value.replace('Φ', 'φ')  # Normalize to lowercase phi
                            if '^' in op_value:
                                power = float(op_value.split('^')[1])
...
def parse_phiflow(code: str) -> Tuple[Dict[str, Dict[str, Any]], List[StateTransition]]:
    """
    Parse φFlow DSL code.
